...
describe 'Example Test - Passing' do
  before(:all) do
  ...
  it 'creates a HTML file in the reports directory' do
    expect(File.exist?("#{FigNewton.test_report_path}/example-test-2.html")).to be true
  end

  it "contains a link with text 'Back to report overview'" do
    visit '/example-test-2.html'
  ...
  it 'displays the correct results for the example group in the Report table' do
    ...
    expect(@row_data).to eq(FigNewton.example_test_passed)
  end
  ...
  it 'displays the correct title in the Example table' do
    visit '/example-test-2.html'
    expect(page).to have_css('h3.card-title', text: 'ExampleTest_2 → passes')
  end

  it 'displays passed for the example group in the example table' do
    visit '/example-test-2.html'
    expect(page.all('tbody')[1].find('.badge').text).to eq('passed')
  end
end
